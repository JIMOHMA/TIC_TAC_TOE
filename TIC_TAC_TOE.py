...
def winning_cond(grid): ##for checking all the eight possible winning situations
    ...
    if grid[1][1] != '       ': ##this checks to see if the centre element of the board is not empty
        if grid[0][0] == grid[1][1]: ##checks to see if element in position (0,0) is same as center element
            if grid[2][2] == grid[1][1]: ## checks to see if element in position (2,2) is same as center element
                return True
...
# computer generates a random position to play on the board
def comp_move(grid):
...
def one_user(grid):
    ...
    while Ongoing:
        ...
        current = get_state(grid, row, col) ## this gets the element in the cell which is provided by the user (1,1)
        if current != '       ': ## this checks to see if the cell provided has an entry in it
            ...
            if winning_cond(grid): ## if this if statement is false, means we don't have a winner (possibly a tie)
    ...
    while Ongoing:
        ...
        if current != '       ': ## this checks to see if the cell provided has an entry in it
            ...
            if winning_cond(grid): ## if this if statement is false, means we don't have a winner(a tie)
                ...
                print(player1 + " wins")
                print(print_board(grid))
                print("last player {} played at this coordinate ({},{})".format(player1, coordinate[0], coordinate[1]))
                print("There are " + str(spaces) + " spaces left")
                #Ongoing = False  ## this ends the while loop when a winner emerges
                break
...
def main():
    ...
    grid = [['       ','       ','       '],
            ['       ','       ','       '],
            ['       ','       ','       ']]  ## a list representation of what the board
